...
5.1 修改片段着色器，仅让笑脸图案朝另一个方向看

>这个另一个方向看，你理解为看向左边就可以了，那么就是需要x方向上的映射相反即可

```c++
 FragColor = mix(texture(texture1, TexCoord), texture(texture2, vec2(1 - TexCoord.x, TexCoord.y)), 0.2);
```

5.2 尝试用不同的纹理环绕方式，设定一个从`0.0f`到`2.0f`范围内的（而不是原来的`0.0f`到`1.0f`）纹理坐标。试试看能不能在箱子的角落放置4个笑脸：[参考解答](https://learnopengl.com/code_viewer_gh.php?code=src/1.getting_started/4.4.textures_exercise2/textures_exercise2.cpp)，[结果](https://learnopengl-cn.github.io/img/01/06/textures_exercise2.png)。记得一定要试试其它的环绕方式。

修改TexCoord从`0.0f`到 `2.0f`。
...
![image-20241205132529141](C:\Users\windows\AppData\Roaming\Typora\typora-user-images\image-20241205132529141.png)
...
![image-20241205132642618](C:\Users\windows\AppData\Roaming\Typora\typora-user-images\image-20241205132642618.png)
...
![image-20241205132716441](C:\Users\windows\AppData\Roaming\Typora\typora-user-images\image-20241205132716441.png)
...
![image-20241205132801979](C:\Users\windows\AppData\Roaming\Typora\typora-user-images\image-20241205132801979.png)

5.3 尝试在矩形上只显示纹理图像的中间一部分，修改纹理坐标，达到能看见单个的像素的效果。尝试使用GL_NEAREST的纹理过滤方式让像素显示得更清晰：[参考解答](https://learnopengl.com/code_viewer_gh.php?code=src/1.getting_started/4.5.textures_exercise3/textures_exercise3.cpp)

>计算红框的坐标值即可。

![image-20241205133144554](C:\Users\windows\AppData\Roaming\Typora\typora-user-images\image-20241205133144554.png)
...
![image-20241205133328832](C:\Users\windows\AppData\Roaming\Typora\typora-user-images\image-20241205133328832.png)
...
![image-20241205133338333](C:\Users\windows\AppData\Roaming\Typora\typora-user-images\image-20241205133338333.png)

5.4 使用一个uniform变量作为mix函数的第三个参数来改变两个纹理可见度，使用上和下键来改变箱子或笑脸的可见度：

将`mixValue`作为一个全局变量， `mixValue = 0.2;`
