...
int main()
{
	...
	// 组合解决方案目录和相对路径
	std::filesystem::path vertexPath = std::filesystem::path(SOLUTION_DIR) / "OpenGL" / "ShaderSource" / "Chapter_9_Color" / "Shader_9_Cast_Multiple lights.vs";
	std::filesystem::path fragmentPath = std::filesystem::path(SOLUTION_DIR) / "OpenGL" / "ShaderSource" / "Chapter_9_Color" / "Shader_9_Cast_Multiple lights.fs";
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightShader.use();
		lightShader.setMatrix4("projection", 1, GL_FALSE, glm::value_ptr(projection));
		lightShader.setMatrix4("view", 1, GL_FALSE, glm::value_ptr(view));
		...
		for (unsigned int i = 0; i < 4; i++)
		{
			...
			lightShader.setMatrix4("model", 1, GL_FALSE, glm::value_ptr(model));
			glDrawArrays(GL_TRIANGLES, 0, 36);
		}
