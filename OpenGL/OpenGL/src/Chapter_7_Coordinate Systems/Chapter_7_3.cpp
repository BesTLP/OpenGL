...
float mixValue = 0.2f;
float fov = 45.0f;
glm::vec3 viewVec3(0.0f, 0.0f, -3.0f);
...
int main()
{
	...
	// 组合解决方案目录和相对路径
	std::filesystem::path vertexPath = std::filesystem::path(SOLUTION_DIR) / "OpenGL" / "ShaderSource" / "Chapter_7_Coordinate Systems" / "Shader_7_3.vs";
	std::filesystem::path fragmentPath = std::filesystem::path(SOLUTION_DIR) / "OpenGL" / "ShaderSource" / "Chapter_7_Coordinate Systems" / "Shader_7_3.fs";
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			view = glm::translate(view, viewVec3);
