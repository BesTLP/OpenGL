...
int main()
{
	...
	// 组合解决方案目录和相对路径
	std::filesystem::path vertexPath = std::filesystem::path(SOLUTION_DIR) / "OpenGL" / "ShaderSource" / "Chapter_6_Transformation" / "Shader_6_1.vs";
	std::filesystem::path fragmentPath = std::filesystem::path(SOLUTION_DIR) / "OpenGL" / "ShaderSource" / "Chapter_6_Transformation" / "Shader_6_1.fs";
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		shader.setMatrix4("transform", 1, GL_FALSE, glm::value_ptr(trans));
		glBindVertexArray(VAO);
